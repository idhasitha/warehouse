...
export const areaRouter = createTRPCRouter({
  ...
  newArea: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        return await ctx.prisma.areas.create({
          data: {
            x: input.x,
            y: input.y,
            width: input.width,
  ...
  // Get all aisles
  allAreas: publicProcedure.query(async ({ ctx }) => {
    try {
      return await ctx.prisma.areas.findMany({
        select: {
          id: true,
          x: true,
          y: true,
  ...
  updateArea: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const { id } = input;
      try {
        return await ctx.prisma.areas.update({
          where: {
            id: id,
          },
          data: {
  ...
  deleteArea: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const { id } = input;
      try {
        return await ctx.prisma.areas.delete({
          where: {
            id: id,
          },
        });
